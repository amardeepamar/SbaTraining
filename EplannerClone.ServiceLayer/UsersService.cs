...
namespace EplannerClone.ServiceLayer
{
    ...
    {
        int InsertUser(UserViewModel uvm);
        List<SelectListItem> GetRoles(); // New method
        void UpdateUserDetails(EditUserDetailsViewModel uvm);
        void UpdateUserPassword(EditUserPasswordViewModel uvm);
        void DeleteUser(int uid);
        List<UserViewModel> GetUsers();
        UserViewModel GetUsersByUsernameAndPassword(string Username, string Password,int RoleId);
    ...
    public class UsersService : IUsersService
    {
        ...
        public int InsertUser(UserViewModel uvm)
        {
            ...
            u.RoleId = uvm.RoleId; // Assign RoleId
        ...
        {
            return ur.GetRoles().Select(r => new SelectListItem
            {
        ...
        public UserViewModel GetUsersByUsernameAndPassword(string Username, string Password, int RoleId)
        {
            User u = ur.GetUsersByUsernameAndPassword(Username, SHA256HashGenerator.GenerateHash(Password)).FirstOrDefault();
            UserViewModel uvm = null;
            if (u != null)
            {
                var config = new MapperConfiguration(cfg => { cfg.CreateMap<User, UserViewModel>(); cfg.IgnoreUnmapped(); });
