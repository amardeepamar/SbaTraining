...
{
    public class SpecialDAO : EpContext
    {
        ...
        public static IEnumerable<SelectListItem> GetAcForDdl()
        {
            IEnumerable<SelectListItem> RolesList = db.tblAcs.Where(x => x.IsDeleted == false).OrderByDescending(x => x.AddDate).Select(x => new SelectListItem()
            {
                Text = x.AC_NAME_EN,
                Value = SqlFunctions.StringConvert((double)x.AC_NO).Trim()
            }).ToList();
            return RolesList;
        }
        public static IEnumerable<SelectListItem> GetAcForDdl(int distid)
        {
            IEnumerable<SelectListItem> RolesList = db.tblAcs.Where(x => x.IsDeleted == false).OrderByDescending(x => x.AddDate).Select(x => new SelectListItem()
            {
                Text = x.AC_NAME_EN,
        ...
        //internal static IEnumerable<SelectListItem> GetAllState()
        //{
        //    IEnumerable<SelectListItem> StatesList = db.StateMasters.Where(x => x.IsDeleted == false).OrderByDescending(x => x.AddDate).Select(x => new SelectListItem()
        //    {
        //        Text = x.StateNameEn,
        //        Value = SqlFunctions.StringConvert((double)x.Id).Trim()
        //    }).ToList();
